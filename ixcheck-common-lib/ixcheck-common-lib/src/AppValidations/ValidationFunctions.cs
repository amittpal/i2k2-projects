...
namespace IXCheckCommonLib.AppValidations
{
    ...
    | RegexOptions.Compiled
        ...
        /// <param name="_email"></param>
        /// <returns>bool</returns>
        public static bool IsValidTan(string _pan, bool _useRegex)
        {
            try
            {
                if (_useRegex)
                {
                    Regex regex = new Regex(@"[A-Za-z]{4}[0-9]{5}[A-Za-z]{1}");
                    Match match = regex.Match(_pan);
                    if (match.Success)
                        return true;
                    else
                        return false;
        ...
        public static bool IsValidPassword(string _inputPassword)
        {
            string _pattern = @"^.*(?=.{8,24})(?=.*[a-z])(?=.*?[0-9])(?=.*[A-Z])(?=.*[@#$%?^&+=])[a-zA-Z0-9@#$%?^&+=]{8,24}$";
            // This regex allows 8 to 24 char, one upper case, one lower case, one numeric value, one special char
            Regex rg = new Regex(_pattern);
            bool _isMatch = rg.IsMatch(_inputPassword);
            return _isMatch;
        }
