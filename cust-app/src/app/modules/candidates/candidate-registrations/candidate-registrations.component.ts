...
@Component({
  selector: 'app-candidate-registrations',
  templateUrl: './candidate-registrations.component.html',
  styleUrls: ['./candidate-registrations.component.scss']
})
export class CandidateRegistrationsComponent implements OnInit {
 public items = [];
  public itemCount = 0;
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("Candidate Registrations");
  }
  ngOnDestroy() {
    this.config.data = [];
  ...
  public updateService(id, name) {
    localStorage.setItem('registrationId', id);
    localStorage.setItem('registrationName', name);
    localStorage.removeItem('statusGUID');
  }
  public finalize(guid) {
    let keyData = [
      {
     ...
     this.restService.ApiEndPointUrlOrKey = Candidates.finalizeRegistration;
