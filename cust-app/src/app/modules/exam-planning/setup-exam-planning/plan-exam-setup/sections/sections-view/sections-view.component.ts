...
export class SectionsViewComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private configService: AppsettingsConfService, private SharedService: SharedService, private router: Router, private primaryHeader: PrimaryHeaderService,
  ...
  getDurationAllowStatus() {
    this.addSectionsFormGroup.get('allow_duration').valueChanges.subscribe(data => {
  ...
  public formSubmit() {
    ...
    if (this.addSectionsFormGroup.valid === false) {
    ...
    } else if (this.totalQuestion !== parseInt(this.addSectionsFormGroup.getRawValue().question_count)) {
      this.messageService.ok("Total number of question should be equal to question count");
      return false;
      ...
      // console.log(JSON.stringify(this.questionJson));
  ...
  onDragged(dragEvent: any) {  //Avinash - 10/12/2019
    const [e, el] = dragEvent.args;
    // do something
  }

  public deleteRow(event: any) {
    // console.log("delete call ",event);
    // this.calTotalAmount();
  }
  ...
  public getChildComponentData(childData: any) {
    ...
    // childData['duration_um'] = childData.duration_uom_guids.name;
  ...
  getSectionDetails(sectionId) {
    // console.log("iam getSectionDetails");
    ...
    this.restService.callApi(keyData)
      .subscribe(sucessResponse => {
        ...
        console.error('ERROR: ', errorResponse['httpErrorResponse']['http_status']);
      });
  }

  resetIntialData() {
    // console.log("i am resetIntialData");
  ...
  setIntialData(value: any) {
    ...
    // this.selectedSubject = value['subjects'];
    ...
    //////
    this.addSectionsFormGroup.get('question_count').disable();
    this.addSectionsFormGroup.get('allow_duration').disable();
    this.addSectionsFormGroup.get('duration').disable();
    this.addSectionsFormGroup.get('auto_switch').disable();
    this.addSectionsFormGroup.get('duration_uom_guid').disable();
    this.addSectionsFormGroup.get('section_name').disable();
