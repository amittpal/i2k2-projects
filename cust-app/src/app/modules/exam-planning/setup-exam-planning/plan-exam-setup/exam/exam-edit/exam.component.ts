...
export class ExamComponent implements OnInit {
    ...
    this.restService.callApi(keyData).subscribe(
      sucessResponse => {
        ...
        if (sucessResponse.exam_setup) {
          ...
          this.addExamFormGroup.controls["question_paper_randomization"].setValue(this.questionRandomizationOptionList[1]["randomization_option_guid"]);
          this.addExamFormGroup.controls["question_paper_randomization"].disable();
        }
      },
    ...
    this.restService.callApi(keyData).subscribe(
      successResponse => {
        if (successResponse["http_status"] == 200) {
          this.messageService.ok("Section Added Successfully");
          this.getData(this.examId, this.Id);
        }
      },
      errorResponse => {
  ...
  getIntialData() {
    ...
    this.restService.callApi().subscribe(
      successResponse => {
        ...
        // console.log(this.questionRandomizationOptionList[1]["randomization_option_guid"])
        this.addExamFormGroup.controls["question_paper_randomization"].setValue(this.questionRandomizationOptionList[1]["randomization_option_guid"]);
        this.addExamFormGroup.controls["question_paper_randomization"].disable();
      },
      errorResponse => {
  ...
  public initializeFields() {
    this.addExamFormGroup = new FormGroup({
      ...
      shifts: new FormControl("", [Validators.required,Validators.min(1), Validators.max(999)]),
      duration: new FormControl("", [Validators.required,Validators.min(1), Validators.max(999)]),
      sections: new FormControl("", [Validators.required,Validators.min(1), Validators.max(99)]),
      primary_language_guid: new FormControl("", Validators.required),
      //dual_show_primary: new FormControl("", Validators.required),
      dual_show_primary: new FormControl({value:0,disabled:true}),
  ...
  public loadsFields(result: any) {
    ...
    this.addExamFormGroup.get("sections").enable();
      ...
      if (Object.keys(this.appRoutes).length !== 0) {
        ...
        if (this.newUser) {
          ...
          this.restService.callApi(keyData).subscribe(
            sucessResponse => {
              this.messageService
                .okRedirectModal(sucessResponse, "SUCCESS", "Go to next tab")
                .subscribe(result => {
                  if (result == true) {
