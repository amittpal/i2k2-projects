...
@Component({
  selector: 'app-mock-test-exam-list',
  templateUrl: './mock-test-exam-list.component.html',
  styleUrls: ['./mock-test-exam-list.component.scss']
})
export class MockTestExamListComponent implements OnInit {
  ...
  ngOnInit() {
    this.primaryHeader.pageTitle.next("MOCK QUESTIONS");
  }
  ngOnDestroy() {
    this.config.data = [];
  ...
  public updateTable(data: any) {              
    this.offset = 0;    
    this.searchFilter.exam_filter.exams = data.mockTest;
