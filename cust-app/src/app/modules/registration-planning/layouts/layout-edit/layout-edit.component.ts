...
export class LayoutEditComponent implements OnInit {
  ...
  initializeLayoutFormValues() {
    ...
    this.initialLayoutFormGroup = new FormGroup({
      layoutCode: new FormControl(this.layoutDetails.layout.layout_code, [Validators.required]),
      layoutName: new FormControl(this.layoutDetails.layout.name, [Validators.required]),
      layoutRegType: new FormControl(this.layoutDetails.layout.layout_registration_guid, [Validators.required]),
  ...
  onComponentSearch(componentName: string) {
    if (componentName) {
      this.searchedComponents = this.componentsList.ixcheckComponents[0].components.filter((c: any) => c.value.toLowerCase().indexOf(componentName.toLowerCase()) !== -1)
    }
    else {
      this.searchedComponents = this.componentsList.ixcheckComponents[0].components;
    }
  ...
  openComponentSettingsModal(pageId: any, sectionId: any, componentId: any, componentSettingsModalTemplate: TemplateRef<any>) {
    ...
    this.addedSections = this.layoutDetails.layout.pages.find(p => p.page_guid === pageId).sections;
    ...
    //adding url for NgxIxcheckPhotouploaderAi
    if (this.componentObject.comp_name === "NgxIxcheckPhotouploaderAi") {
      const aiComponentJson = this.componentsList.ixcheckComponents[0].components.find(c => c.componentName === "NgxIxcheckPhotouploaderAi");
      if (aiComponentJson) {
        this.componentObject.ai_parameters.algo_url = aiComponentJson.ai_parameters.algo_url;
        this.componentObject.ai_parameters.color_url = aiComponentJson.ai_parameters.color_url;
      }
  ...
  saveTemplateChanges() {
    if (this.initialLayoutFormGroup.valid) {
      ...
      this.restService.ApiEndPointUrlOrKey = Registration.editLayout;
