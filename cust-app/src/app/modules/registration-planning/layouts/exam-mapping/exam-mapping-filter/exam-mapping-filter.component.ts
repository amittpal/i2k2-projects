...
@Component({
  selector: 'app-exam-mapping-filter',
  templateUrl: './exam-mapping-filter.component.html',
  styleUrls: ['./exam-mapping-filter.component.scss']
})

export class ExamMappingFilterComponent implements OnInit {
  ...
  ngOnInit() {    
    let filters = this.filterService.getFilter("examMappingFilter");
    if (filters === undefined) {
      this.filterService.addFilter("layout");
      filters = this.filterService.getFilter("examMappingFilter");
    }
    this.initFormGroup();
  ...
  public initFormGroup() {
    this.layoutFormGroup = new FormGroup({
      layout_number: new FormControl('', Validators.minLength(3)),
      ...
      exam_type_name: new FormControl(''),            
      layout_status: new FormControl('')
    });
    let savedfilter = this.filterService.getFilter("examMappingFilter");
  ...
  public updateFilterService() {
    let filters = this.filterService.getFilter("examMappingFilter");
    if (filters) {
      this.filterService.updateFilter(this.searchFilter.layout, 0, "examMappingFilter");
    }
  }
