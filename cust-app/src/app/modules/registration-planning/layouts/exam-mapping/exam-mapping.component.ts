...
@Component({
  ...
  styleUrls: ['./exam-mapping.component.scss']
})
...
export class ExamMappingComponent implements OnInit {
  ...
  ngOnInit() {
    this.primaryHeader.pageTitle.next("LAYOUT MAPPING");
  }
  ngOnDestroy() {
    this.config.data = [];
  ...
  public getData(params?: NgxIxcheckTableParams) {
    ...
    if (Object.keys(this.appRoutes).length !== 0) {
      this.restService.ApiEndPointUrlOrKey = Registration.getExamMappingList;
