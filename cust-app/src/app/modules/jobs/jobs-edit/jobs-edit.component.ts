...
export class JobsEditComponent implements OnInit {
  ...
  public updateJobsFormGroup: FormGroup;
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("Job Edit");
  ...
  public initializeFields() {
    this.updateJobsFormGroup = new FormGroup({
      id: new FormControl(''),
  ...
  getData(id: any) {
    ...
    this.restService.callApi(keyData)
      .subscribe(sucessResponse => {
        this.orgionalData = Object.assign([], sucessResponse.jobs[0]);
        this.setInitializeFields(sucessResponse.jobs[0]);
        this.loadingData = sucessResponse.jobs[0];
      });
  }
  ...
  public setInitializeFields(result: any) {
    this.updateJobsFormGroup = new FormGroup({
      ...
      parent_job_id: new FormControl(result.parent_job_id),
  ...
  private getParams() {
    let params = this.updateJobsFormGroup.getRawValue();
    return params;
  }
  getJobTypeList() {
  ...
  public formSubmit() {
    if (this.updateJobsFormGroup.valid === false) {
      let form = document.getElementById('updateJobsForm');
      form.classList.add('was-validated');
    } else if (this.updateJobsFormGroup.valid === true) {
  ...
  public reset() {
    ...
    this.updateJobsFormGroup.reset();
    this.initializeFields();
    let params = this.getParams();
  }
