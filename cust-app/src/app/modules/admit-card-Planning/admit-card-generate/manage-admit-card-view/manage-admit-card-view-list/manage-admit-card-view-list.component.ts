...
@Component({
  selector: 'app-manage-admit-card-view-list',
  templateUrl: './manage-admit-card-view-list.component.html',
  styleUrls: ['./manage-admit-card-view-list.component.scss']
})
export class ManageAdmitCardViewListComponent implements OnInit {
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("ADMIT CARDS VIEW");
  ...
  private getData(params: NgxIxcheckTableParams, fromSubmit: boolean) {
    ...
    if (Object.keys(this.appRoutes).length !== 0) {
      this.restService.ApiEndPointUrlOrKey = AdmitCard.getAdmitCardList;      
