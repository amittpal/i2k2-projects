...
export class SetupComponent implements OnInit {
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("PACKAGING & DISTRIBUTION SETUP");
  }
  ...
  public updateTable(data) {
    this.offset = 0;
    this.search_config.exam_filter.exam_list = data.exams;
  ...
  private getData(params: NgxIxcheckTableParams, fromSubmit: boolean) {
    ...
    if (Object.keys(this.appRoutes).length !== 0) {
      this.restService.ApiEndPointUrlOrKey = Packaging.getPackageSetupExamList;
  ...
  public refreshExamList()
  {
    this.notFound = false;
    this.resetFilterFlag = false;
    let examSearchParams = this.getSearchParams(this._getRemoteParameters(), false);
    //call api code here...
    if (Object.keys(this.appRoutes).length !== 0) {
      this.restService.ApiEndPointUrlOrKey = Packaging.refreshPackageSetupExamList;
      this.restService.HttpPostParams = examSearchParams;
      this.restService.AlertAndErrorAction = HandelError.HideAndReturn;
      this.restService.ShowLoadingSpinner=true;
      this.restService.callApi()
