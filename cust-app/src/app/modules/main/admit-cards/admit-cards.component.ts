...
@Component({
  ...
  styleUrls: ['./admit-cards.component.scss']
})
export class AdmitCardsComponent implements OnInit {

  @Input() regGuid: any;
  @Input() examGuid: any;
  public admitCardDetail = "";
  constructor(
  ...
  getAdmitCardDetails() {
    ...
    this.restService.ApiEndPointUrlOrKey = Main.getAdmitCardDetail;
    this.restService.ApiEndPointMehod = RestMethods.Get;
    this.restService.callApi(keyData).subscribe(
      successResponse => {
        this.admitCardDetail = successResponse.admit_card[0];
      })
  }
