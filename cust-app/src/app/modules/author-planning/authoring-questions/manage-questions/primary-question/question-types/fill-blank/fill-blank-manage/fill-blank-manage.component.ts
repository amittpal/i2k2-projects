...
@Component({
  selector: 'app-fill-blank-manage',
  templateUrl: './fill-blank-manage.component.html',
  styleUrls: ['./fill-blank-manage.component.scss']
})
export class FillBlankManageComponent implements OnInit {
  ...
  initializeForm() {
    ...
    if (details) {  
      ...
      this.questionAddFormGroup = new FormGroup({
        ...
        answer: new FormControl(null,Validators.compose([Validators.required,Validators.maxLength(200)]))
        //this.QuestionDetails.answer[0].option_text
      })
    }
  }
  ...
  setupInitialDetails() {           
    ...
    this.initializeForm();
     //updating answer if exists
     if(this.QuestionDetails.answer.length>0)
     {
       this.questionAddFormGroup.get('answer').setValue(this.QuestionDetails.answer[0].option_text);
     }  
  }
  ...
  onAddQuestionFormsubmit() {   
    if (this.questionAddFormGroup.valid) {
      ...
      // if (this.QuestionDetails.question_references.length == 0) {
      //   this.messageService.ok('Please add reference for question answer');
      //   return;
      // }
      // else
      // {
      //   //reference data must not be empty
      //   if(this.QuestionDetails.question_references[0].reference==="" || this.QuestionDetails.question_references[0].remark==="")
      //   {
  ...
  addParams() {
    let answer=[
      {
        "option_text":this.questionAddFormGroup.get('answer').value,
