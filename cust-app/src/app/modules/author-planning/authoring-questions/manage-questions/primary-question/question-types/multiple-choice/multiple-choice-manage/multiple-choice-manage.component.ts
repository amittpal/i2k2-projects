...
@Component({
  selector: 'app-multiple-choice-manage',
  templateUrl: './multiple-choice-manage.component.html',
  styleUrls: ['./multiple-choice-manage.component.scss']
})
export class MultipleChoiceManageComponent implements OnInit {
  ...
  ngOnChanges() {
    this.clearForm();
    this.setupInitialDetails(this.QuestionDetails);
  ...
  setupInitialDetails(response: any) {
    ...
    this.QuestionDetails.options.forEach(element => {
      if (itemDetails === element) {
        if (event.target.checked) {
          ...
          let mappedObject = Object.assign(element, { "option_guid": element.guid });
          this.QuestionDetails.answer.pop(mappedObject);
        }
