...
import { RegistrationsSetup, Author } from '../../../../shared/enumrations/app-enum.enumerations';
...
export class QuestionsListComponent implements OnInit {
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("QUESTION");
  }
  ...
  public updateTable(data) {
    this.offset = 0;
    this.search_config.exam_filter.exams = data.exams;
  ...
  private getData(params: NgxIxcheckTableParams, fromSubmit: boolean) {
    ...
    if (Object.keys(this.appRoutes).length !== 0) {
      this.restService.ApiEndPointUrlOrKey = Author.getAuthoringQuestionList;      
  ...
  public updateFilter() {
    ...
    this.updatedFilter = this.search_config.exam_filter.exams;
  ...
  public resetFilter() {
    this.search_config.exam_filter.exams = {}
    this.getData(this._getRemoteParameters(), false);
    this.resetFilterFlag = true;
  }

  public updateSearch(removedId: string) {
    this.search_config.exam_filter.exams[removedId] = "";
    this.getData(this._getRemoteParameters(), false);
    this.count = Number(this.count) + 1;
  }
}
