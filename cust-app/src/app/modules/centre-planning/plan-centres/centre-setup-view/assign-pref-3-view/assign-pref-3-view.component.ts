...
@Component({
  selector: 'app-assign-pref-3-view',
  templateUrl: './assign-pref-3-view.component.html',
  styleUrls: ['./assign-pref-3-view.component.scss']
})
export class AssignPrefCViewComponent implements OnInit {
  @Input() examGuid: any;
  ...
  getIntialData() {
    ...
    this.restService.ApiEndPointUrlOrKey = Centre.getAssignPrefC;
    ...
    this.restService.callApi(keyData).subscribe(successResponse => {
      ...
      this.allocationPercentage = successResponse["distribution"]["percentage"];
      let unique = [...new Set(successResponse.shifts.map(item => item.shift_number))];
