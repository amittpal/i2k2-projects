...
import { HandelError, Centre } from 'src/app/shared/enumrations/app-enum.enumerations';
...
@Component({
  selector: 'app-table-list-c-rowdetail-view',
  templateUrl: './table-list-c-rowdetail-view.component.html',
  styleUrls: ['./table-list-c-rowdetail-view.component.scss']
})
export class TableListCRowdetailViewComponent implements OnInit {
  ...
  examGuid: any;
  centreJson: any = [];
  constructor(private SharedService: SharedService, private route: ActivatedRoute, private restService: GlobalRestService, private authService: AuthService, private filterService: FilterService,
  ...
  getChildComponentData(childData: any) {
    ...
    this.restService.ApiEndPointUrlOrKey = Centre.updateAddCentreDataC;
    ...
    // let centres_guid = childData['centres_guid'];
    // this.items.forEach((element, i) => {

    //   if (element['centres_guid'] == centres_guid) {
  ...
  getCentresList() {
    ...
    this.restService.ApiEndPointUrlOrKey = Centre.getAddedCentreListC;
  ...
  addRow(data: any) {
    ...
    if (item) {
      if (item.is_select === '0') {
        this.items.find(c => c.centres_guid === data.centres_guid).is_select = '1';
      }
      else {
        this.items.find(c => c.centres_guid === data.centres_guid).is_select = '0';
      }
    }
    // let selectCentre = this.items.filter(c => c.is_select = '0')
  }
  sendToParent() {
    this.restService.ApiEndPointUrlOrKey = Centre.updateSelectedCentreListC;
    this.restService.ApiEndPointMehod = RestMethods.Post;
