...
export class ShiftComponent implements OnInit {
  ...
  public formSubmit() {  
    ...
    let validInput = Boolean(this.shiftDetailsParam["shiftsMain"].find((a) => a.exam_date == ""));
    //let validStatus = this.isFormValid();
    let validStatus = true;

    let greaterTotalRegDataDistInput = Boolean(this.items.find((a) => a.totalQuestion > 100));
    let lowerTotalRegDataDistInput = Boolean(this.items.find((a) => a.totalQuestion < 100));

    if (greaterTotalRegDataDistInput) {
      this.messageService.ok(
        "Reg Data Distribution can't be greater than 100% "
      );
    } else if (lowerTotalRegDataDistInput && this.showdistribution) {
      this.messageService.ok("Reg Data Distribution can't be less than 100% ");
    } else if (validInput) {
      this.messageService.ok("Please enter shift information");
    } else if (validStatus === true) {
      let params = this.getParams();
      this.restService.ApiEndPointUrlOrKey = Centre.updateShiftExamMain;
