...
export class CentreEditRowdetailsComponent implements OnInit {
  ...
  intialChildForm() {  
      this.childFromGroup = new FormGroup({
        ...
        allowed_ph_capacity_per: new FormControl("", Validators.required),
        allowed_normal_capacity_per: new FormControl("", Validators.required),
  ...
  sendToParent() {
    if (this.childFromGroup.valid === false) {
      ...
      formdata["pwd_seats"] = Math.floor(Number(this.rowItemData["total_ph_seats"]) * Math.floor(Number(formdata.allowed_ph_capacity_per)) * 0.01);
      formdata["normal_seats"] = Math.floor(Number(this.rowItemData["total_normal_seats"]) * Math.floor(Number(formdata.allowed_normal_capacity_per)) * 0.01);
