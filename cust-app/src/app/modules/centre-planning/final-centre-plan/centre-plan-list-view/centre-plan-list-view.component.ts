...
@Component({
  selector: 'app-centre-plan-list-view',
  templateUrl: './centre-plan-list-view.component.html',
  styleUrls: ['./centre-plan-list-view.component.scss']
})
export class CentrePlanListViewComponent implements OnInit {
  ...
  getSnapshotList(guid) {
    if (Object.keys(this.appRoutes).length !== 0) {
      ...
      this.restService.callApi(keyData).subscribe(
        (sucessResponse) => {
          this.snapShotList = sucessResponse.snap_shot.filter(item => { return (item.planning_status == "Alloted" || item.planning_status == "Finalized") });;
        },
        (errorResponse) => {
          if (errorResponse !== undefined) {
            this.displayMessage =
