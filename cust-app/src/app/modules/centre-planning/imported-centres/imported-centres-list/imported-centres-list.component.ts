...
@Component({
  selector: 'app-imported-centres-list',
  templateUrl: './imported-centres-list.component.html',
  styleUrls: ['./imported-centres-list.component.scss']
})
export class ImportedCentresListComponent implements OnInit {
  ...
  ngOnInit() {
    //setting page title
    this.primaryHeader.pageTitle.next("Imported Centres");
  }
  ngOnDestroy() {
    this.config.data = [];
    ...
    if (Object.keys(this.appRoutes).length !== 0) {
      ...
      this.restService.callApi()
        .subscribe(sucessResponse => {
          this.items = sucessResponse.imported_centres;
          this.notFound = false;
          //paging info
          ...
          this.search_config.imported_centres_filter.paging = sucessResponse.paging;
        }, errorResponse => {
  ...
  public updateFilter() {
    ...
    this.updatedFilter = this.search_config.imported_centres_filter.imported_centres;
  ...
  public resetFilter() {
    this.search_config.imported_centres_filter.imported_centres = {}
    this.getData(this._getRemoteParameters(), false);
  ...
  public onDelete(id: any, name: any) {
    ...
    this.messageService.confirm(["Are you sure you want to delete this record?", JSON.stringify(name)], "Delete", "Yes", "NO", "error").subscribe(result => {
      if (result == true) {
        ...
        let params = { "id": id };
        this.restService.ApiEndPointUrlOrKey = ImportedCentres.deleteImportedCentresById;
